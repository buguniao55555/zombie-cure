...
function Commands.UseAntidote(player, args)
    print("[ZombieCure][SERVER] Use Antidote... ")
    -- 1) Validate player actually has the antidote
    local inv = player:getInventory()
    local item = inv:FindAndReturn("ZombieCure.ZombieCureAntidote")
    if not item then return end  -- anti-cheat: client lied or desynced

    -- 2) Remove item on SERVER and sync removal to client
    inv:Remove(item)
    sendRemoveItemFromContainer(inv, item)  -- required sync :contentReference[oaicite:5]{index=5}

    -- 3) Apply your existing effects HERE (server-side)
    local stats = player:getStats()
    local nutrition = player:getNutrition()
    local bodyDamage = player:getBodyDamage()
    ...
    -- In case of high drunkenness when taking medicine, 33% chance of death, 33% chance of pain, 33% chance of deafness
    elseif drunk >= 20 then
        player:getStats():set(CharacterStat.INTOXICATION, 0.0)
        if random_instance:random(100) <= 33 then
            player:Say(getText("IGUI_ZombieCure_Say_AlcoholBad"))
            player:setHealth(0)
        elseif random_instance:random(67) <= 33 then
...
Events.OnClientCommand.Add(function(module, command, player, args)
    if module == "ZombieCure" and Commands[command] then
        Commands[command](player, args)
    end
end)

local function onAntidoteUse(item, player)
    print("[ZombieCure][CLIENT] Use Antidote... ", tostring(item), tostring(player))
    sendClientCommand(player, "ZombieCure", "UseAntidote", { })
end

local function addAntidoteMenu(player, context, items)
    local item = nil
    ...
    if item:getType() == "ZombieCureAntidote" then
        context:addOption(getText("ContextMenu_ZombieCure_TryCure"), item, onAntidoteUse, getPlayer())
    end
end
...
if ((not (getWorld():getGameMode() == "Multiplayer")) or isServer()) then
    print("[ZombieCure] start dropping meds...");
    Events.OnZombieDead.Add(onZombieDeadDropMeds);
end

Events.OnFillInventoryObjectContextMenu.Add(addAntidoteMenu);